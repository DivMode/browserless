 ...
 */
const buildSchemas = async (externalHTTPRoutes = [], externalWebSocketRoutes = []) => {
  ...
  const { compilerOptions } = JSON.parse(await fs.readFile("tsconfig.json", "utf-8"));
        ...
        return Promise.all(
          schemas.map((schemaName) => {
            if (findExportedInterface(sourceFile, schemaName)) {
              ...
              const schemaSuffix = schemaName.replace("Schema", "").toLocaleLowerCase();
              ...
              try {
                ...
                console.error(`Error generating schema: (${routeName}) (${jsonPath}): ${e}`);
                return null;
              }
            }
            return;
