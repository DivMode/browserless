...
describe("Chromium WebSocket API", function () {
  ...
  it("runs chromium Playwright-CDP requests", async () => {
    ...
    const browser = await chromium.connectOverCDP(`ws://localhost:3000/chromium?token=browserless`);
  ...
  it("rejects playwright without tokens", async () => {
    ...
    await chromium.connect(`ws://localhost:3000/chromium/playwright`).catch((e) => {
      const results = metrics.get();
