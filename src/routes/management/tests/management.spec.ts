...
describe("Management APIs", function () {
  ...
  describe("CORS", () => {
    it("allows Single Origin OPTIONS requests", async () => {
      ...
      const r = await fetch("http://localhost:3000/config?token=6R0W53R135510", {
        method: "OPTIONS",
        headers: {
      ...
      expect(r.headers.get("access-control-allow-origin")).to.equal("https://one.one.one.one");
    });
    ...
    it("allows glob-matched OPTIONS requests with OR patterns", async () => {
      ...
      expect(r.status).to.equal(204);
      expect(r.headers.get("access-control-allow-origin")).to.equal("https://abc.one.one.one.one");
    });
    ...
    it("allows glob-matched OPTIONS requests with OR patterns across two domains", async () => {
      ...
      const r = await fetch("http://localhost:3000/config?token=6R0W53R135510", {
        method: "OPTIONS",
        headers: {
          Origin: "https://deploy-preview-123--funky-monkey-12345.netlify.app",
        },
      });
  ...
  describe("Static Files Serving", () => {
    it("serves docs pages", async () => {
      ...
      await fetch("http://localhost:3000/docs/index.html?token=6R0W53R135510").then(async (res) => {
      ...
      await fetch("http://localhost:3000/docs/docs.js?token=6R0W53R135510").then(async (res) => {
        expect(res.status).to.equal(200);
  ...
  it("allows requests to /active", async () => {
    ...
    await fetch("http://localhost:3000/active?token=6R0W53R135510").then(async (res) => {
      expect(res.headers.get("content-type")).to.equal("text/plain; charset=UTF-8");
  ...
  it("Throws an error trying to kill invalid session", async () => {
    ...
    await fetch(`http://localhost:3000/kill/invalid-session?token=6R0W53R135510`).then(
      async (res) => {
        expect(res.status).to.equal(404);
      },
    );
