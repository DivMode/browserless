...
describe("Chrome WebSocket API", function () {
  ...
  it("runs chrome Playwright-CDP requests", async () => {
    ...
    const browser = await chromium.connectOverCDP(`ws://localhost:3000/chrome?token=browserless`);
  ...
  it("rejects playwright without tokens", async () => {
    ...
    await chromium.connect(`ws://localhost:3000/chrome/playwright`).catch((e) => {
      const results = metrics.get();
