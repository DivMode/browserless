...
export class Config extends EventEmitter {
  ...
  protected concurrent = +(process.env.CONCURRENT ?? process.env.MAX_CONCURRENT_SESSIONS ?? "10");
  protected queued = +(process.env.QUEUE_LENGTH ?? process.env.QUEUED ?? "10");
  protected timeout = +(process.env.TIMEOUT ?? process.env.CONNECTION_TIMEOUT ?? "30000");
  ...
  protected corsMethods = process.env.CORS_ALLOW_METHODS ?? "OPTIONS, POST, GET";
  ...
  public getExternalWebSocketAddress(): string {
    ...
    httpAddress.protocol = httpAddress.protocol.startsWith("https") ? "wss:" : "ws:";
