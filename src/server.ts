...
export class HTTPServer extends EventEmitter {
  ...
  constructor(
    ...
    runForkInServer(
      Effect.logInfo(`Server instantiated with host "${this.host}" on port "${this.port}"`),
    );
  }
  ...
  protected handleRequestEffect(request: http.IncomingMessage, res: http.ServerResponse) {
    const server = this;
    return Effect.fn("server.handleRequest")(function* () {
      ...
      if (server.config.getAllowCORS()) {
        ...
        if (origin && isMatch(origin, corsHeaders["Access-Control-Allow-Origin"])) {
      ...
      const route = yield* Effect.promise(() => server.router.getRouteForHTTPRequest(req));
      ...
      if (route?.auth) {
        ...
        const isPermitted = yield* Effect.promise(() => server.token.isAuthorized(req, route));
      ...
      if (
        ((req.headers["content-type"]?.includes(contentTypes.json) ||
          (route.accepts.length === 1 && route.accepts.includes(contentTypes.json))) &&
          typeof body !== "object") ||
        body === null
      ) {
        ...
        try {
          ...
          if (valid.error) {
            const errorDetails = valid.error.details
              .map(
                ({ message, context }: { context?: { message: string }; message: string }) =>
                  context?.message || message,
              )
              .join("\n");

            yield* Effect.logError(`HTTP query-params contain errors sending 400:${errorDetails}`);
      ...
      if (route.bodySchema) {
        ...
        try {
          ...
          if (valid.error) {
            ...
            yield* Effect.logWarning(`HTTP body validation failed:${errorDetails}`);
      ...
      yield* Effect.promise(() => (route as HTTPRoute).handler(req, res));
      yield* Effect.logTrace("HTTP connection complete");
    })();
  }

  protected async handleRequest(request: http.IncomingMessage, res: http.ServerResponse) {
    return Effect.runPromise(this.handleRequestEffect(request, res)).catch((e) =>
      this.handleErrorRequest(e instanceof Error ? e : new Error(String(e)), res),
    );
  }
  ...
  protected handleWebSocketEffect(
    ...
    return Effect.fn("server.handleWebSocket")(function* () {
      ...
      yield* Effect.logTrace(`Handling inbound WebSocket request on "${request.url || ""}"`);
      ...
      const route = yield* Effect.promise(() => server.router.getRouteForWebSocketRequest(req));

      if (route) {
        yield* Effect.logTrace(`Found matching WebSocket route handler "${route.path}"`);
        ...
        if (route?.auth) {
          yield* Effect.logTrace(`Authorizing WebSocket request to "${req.parsed.href}"`);
          const isPermitted = yield* Effect.promise(() => server.token.isAuthorized(req, route));
        ...
        if (route.querySchema) {
          ...
          try {
            ...
            if (valid.error) {
              ...
              yield* Effect.logWarning(`WebSocket query-params validation failed:${errorDetails}`);
        ...
        yield* Effect.promise(() => (route as WebSocketRoute).handler(req, socket, head));
        yield* Effect.logTrace("Websocket connection complete");
        return;
      }

      yield* Effect.logWarning(`No matching WebSocket route handler for "${req.parsed.href}"`);
  ...
  protected async handleWebSocket(
    ...
    return Effect.runPromise(this.handleWebSocketEffect(request, socket, head)).catch((e) =>
      this.handleErrorRequest(e instanceof Error ? e : new Error(String(e)), socket),
    );
  }
  ...
  public shutdownEffect() {
    return Effect.fn("server.shutdown")({ self: this }, function* () {
      ...
      yield* Effect.promise(() => new Promise<void>((r) => this.server?.close(() => r())));
