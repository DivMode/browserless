    ...
    Effect.sync(() => {
      Match.value(event).pipe(
        Match.tag("Detected", ({ active }) => {
          deps
            ...
            .catch((e) =>
              runForkInServer(
                Effect.logDebug(
                  `emitDetected failed: ${e instanceof Error ? e.message : String(e)}`,
                ),
              ),
            );
        }),
        ...
        Match.tag("Progress", ({ active, state, extra }) => {
          ...
          deps
            ...
            .catch((e) =>
              runForkInServer(
                Effect.logDebug(
                  `emitProgress failed: ${e instanceof Error ? e.message : String(e)}`,
                ),
              ),
            );
        ...
        Match.tag("Solved", ({ active, result, cf_summary_label, skipMarker }) => {
          ...
          const timingStr =
            snap.checkbox_to_click_ms != null
              ? ` checkbox_to_click_ms=${snap.checkbox_to_click_ms} phase4_ms=${snap.phase4_duration_ms}`
              : "";
          runForkInServer(
            Effect.logInfo(
              `CF solved: session=${deps.sessionId.slice(0, 8)} type=${result.type} method=${result.method} duration=${result.duration_ms}ms${timingStr}`,
            ),
          );
          deps
            ...
            .catch((e) =>
              runForkInServer(
                Effect.logDebug(`emitSolved failed: ${e instanceof Error ? e.message : String(e)}`),
              ),
            );
          if (!skipMarker) {
        ...
        Match.tag(
          "Failed",
          ({ active, reason, duration, phaseLabel, cf_summary_label, skipMarker, cf_verified }) => {
            ...
            const diagStr = diag
              ? ` diag_alive=${diag.alive} diag_cbI=${diag.cbI} diag_inp=${diag.inp} diag_shadow=${diag.shadow} diag_bodyLen=${diag.bodyLen}`
              : "";
            const timingStr =
              snap.checkbox_to_click_ms != null
                ? ` checkbox_to_click_ms=${snap.checkbox_to_click_ms} phase4_ms=${snap.phase4_duration_ms}`
            ...
            deps
              ...
              .catch((e) =>
                runForkInServer(
                  Effect.logDebug(
                    `emitFailed failed: ${e instanceof Error ? e.message : String(e)}`,
                  ),
                ),
              );
            if (!skipMarker) {
              marker(active.pageTargetId, "cf.failed", {
                reason,
                duration_ms: duration,
                phase_label,
                oopif_url: active.info.url,
                rechallenge: isRechallenge,
                cf_verified: cfVerified,
              });
            }
          },
        ...
        Match.tag("StandaloneAutoSolved", ({ targetId, signal, tokenLength, cdpSessionId }) => {
          ...
          runForkInServer(
            Effect.logInfo(
              `CF solved: session=${deps.sessionId.slice(0, 8)} type=turnstile method=auto_solve duration=0ms`,
            ),
          );
          deps
            .emitClientEvent()("Browserless.cloudflareSolved", {
