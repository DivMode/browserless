...
const detectCF = (url: string): InterstitialCFType | null => {
  if (url.includes("__cf_chl_rt_tk=") || url.includes("challenges.cloudflare.com"))
    return "interstitial";
  return null;
};
...
effectDescribe("classifyNavigationOutcome", () => {
  ...
  effectIt.effect("TurnstileToCF — turnstile host page navigates to CF URL", () =>
    Effect.gen(function* () {
      ...
      const outcome = yield* classifyNavigationOutcome(
        active,
        "https://challenges.cloudflare.com/something",
        "",
        detectCF,
      );
  ...
  effectIt.effect("TurnstileSolved — turnstile host page navigates to clean URL", () =>
    Effect.gen(function* () {
      const active = makeActive({
        type: "turnstile",
        clickDelivered: true,
        clickDeliveredAt: Date.now() - 2000,
      });
      const outcome = yield* classifyNavigationOutcome(
        active,
  ...
  effectIt.live("RechallengeLimitReached — too many rechallenges", () =>
    Effect.gen(function* () {
      const active = makeActive({
        type: "interstitial",
        clickDelivered: true,
        rechallengeCount: MAX_RECHALLENGES - 1,
      });
      const outcome = yield* classifyNavigationOutcome(
        active,
        "https://example.com?__cf_chl_rt_tk=abc",
  ...
  effectIt.live(
    "CosmeticUrlChange — same origin+path, only query params stripped (the 67040.info bug)",
    () =>
      Effect.gen(function* () {
  ...
  effectIt.live(
    "InterstitialSolved — same-origin same-path after cosmeticNavSeen (the 2captcha-cf fix)",
    () =>
      Effect.gen(function* () {
  ...
  effectIt.live("CosmeticUrlChange — same-origin same-path with CF title is cosmetic", () =>
    Effect.gen(function* () {
      ...
      const active = makeActive({
        type: "interstitial",
        url: "https://example.com/?__cf_chl_rt_tk=x",
      });
      const outcome = yield* classifyNavigationOutcome(
        active,
        "https://example.com/",
  ...
  effectIt.live(
    "InterstitialSolved — same-origin same-path with non-CF title is a real solve (2captcha-cf)",
    () =>
      Effect.gen(function* () {
        ...
        const active = makeActive({
          type: "interstitial",
          url: "https://2captcha.com/demo/cloudflare-turnstile-challenge?__cf_chl_rt_tk=abc",
        });
        const outcome = yield* classifyNavigationOutcome(
          active,
          "https://2captcha.com/demo/cloudflare-turnstile-challenge",
          "2captcha Turnstile Challenge Demo",
          noCF,
        );
        expect(outcome._tag).toBe("InterstitialSolved");
      }),
  ...
  effectIt.live(
    "InterstitialSolved — different origin is NOT cosmetic (cross-origin redirect)",
    () =>
      Effect.gen(function* () {
        ...
        const active = makeActive({
          type: "interstitial",
          clickDelivered: true,
          url: "https://challenges.cloudflare.com/page",
        });
        const outcome = yield* classifyNavigationOutcome(
          active,
          "https://example.com/",
