...
describe("Replay Recording Health", () => {
  ...
  it("per-tab replays store events and CF markers", async () => {
    ...
    try {
      ...
      await page
        .goto("https://peet.ws/turnstile-test/non-interactive.html", {
          waitUntil: "load",
          timeout: 10_000,
        })
        .catch(() => {});
    ...
    expect(
      summary,
      "No summary from /signals or cfSummary — solver or marker pipeline broken",
    ).not.toBeNull();
    console.log(
    ...
    for (const r of replays) {
      ...
      if (tabHasDetection) {
        expect(
          signals!.summary,
          `${r.id}: has cf.detected but /signals summary is null`,
        ).not.toBeNull();
        ...
        const hasClick = signals!.cf_markers.some(
