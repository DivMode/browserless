        ...
        Effect.gen(function* () {
          ...
          try {
            ...
            if (site.expectedTypes.includes("turnstile") && summary!.type === "turnstile") {
              const widgetRendered = allMarkers.some(
                (m) =>
                  m.tag === "cf.oopif_discovered" ||
                  (m.tag === "cf.phase2_end" && m.payload.found === true),
              );
              if (!widgetRendered) {
                const replayUrl = replayId ? `${REPLAY_HTTP}/replays/${replayId}` : null;
                failWithEvidence(
            ...
            // NOTHING is optional — every value MUST exist. Missing = hard failure.
            {
              ...
              const fail = (msg: string): never =>
                failWithEvidence(site.name, msg, allMarkers, replayUrl);

              // 6a. Signals already fetched in step 2 — validate they have data
              if (!signals)
                throw fail(`/signals returned null for replay ${replayId} — endpoint down or 404`);
              if (!signals.summary)
                throw fail(
                  `/signals has no summary but server cfSummary label is '${summary!.label}'`,
                );
              ...
              if (signals.cf_marker_count === 0)
                throw fail("/signals cf_marker_count=0 — no CF markers");
              ...
              const positiveSolves = nonCleanupSolves.filter(
                (s) => Number(s.payload.duration_ms) > 0,
              );
              const zeroSolves = nonCleanupSolves.filter(
                (s) => Number(s.payload.duration_ms) === 0,
              );
              ...
              const lifecycle = signals.cf_markers
                .filter(
                  (m) => m.tag === "cf.detected" || m.tag === "cf.solved" || m.tag === "cf.failed",
                )
              ...
              if (orphaned.length > 0) {
                const info = orphaned
                  .map((d) => `+${d.offset_ms}ms type=${d.payload.type}`)
                  .join(", ");
                throw fail(
                  `${orphaned.length} orphaned cf.detected (no solve/fail before next detection): ${info}`,
                );
              }

              // 6g. Bidirectional click consistency — both directions are mandatory
              const hasClickMarker = signals.cf_markers.some(
                (m) => m.tag === "cf.oopif_click" && m.payload.ok,
              );
