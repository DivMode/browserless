...
describe("phase2OOPIFResolution — polling behavior", () => {
   ...
   */
  it.live(
    "REGRESSION: empty candidates without iframeFrameId — must keep polling until OOPIF appears",
    () =>
      Effect.gen(function* () {
        const { layer } = makeMockEvents();
  ...
  it.live(
    "REGRESSION: empty candidates without iframeFrameId — polls ALL when OOPIF never appears",
    () =>
      Effect.gen(function* () {
        const { layer } = makeMockEvents();
        const mock = makeMockSend({ appearOnPoll: Infinity });
