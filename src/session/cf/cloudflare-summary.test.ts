...
import { buildSummaryFromMarkers, assertSummaryConsistency, CF_MARKERS } from "./cf-summary.js";
...
const marker = (
  ...
  timestamp = 0,
): ReplayMarker => ({ tag, payload, timestamp });
...
describe("deriveSolveAttribution", () => {
  it.each(
    ...
    "$signal + clickDelivered=true → $expectedMethod, $expectedLabel",
    ({ signal, click, expectedMethod, expectedLabel }) => {
  ...
  it.each(
    ...
    "$signal + clickDelivered=false → $expectedMethod, $expectedLabel",
    ({ signal, click, expectedMethod, expectedLabel }) => {
      const result = deriveSolveAttribution(signal, click);
...
describe("buildSummaryFromMarkers", () => {
  ...
  it("Emb? — embedded detected only, no solve/fail", () => {
    const markers = [marker(CF_MARKERS.DETECTED, { type: "turnstile" })];
    expect(buildSummaryFromMarkers(markers)!.label).toBe("Emb?");
  });
  ...
  it("multi-phase — Int✓ Emb→ compound label", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.SOLVED,
        { method: "click_navigation", signal: "page_navigated", phase_label: "✓" },
        100,
      ),
      marker(CF_MARKERS.DETECTED, { type: "turnstile" }, 200),
      marker(
        CF_MARKERS.SOLVED,
        { method: "auto_solve", signal: "bridge_solved", phase_label: "→" },
        300,
      ),
    ];
    const result = buildSummaryFromMarkers(markers)!;
  ...
  it.each(["non_interactive", "invisible"] as const)("type=%s → Emb→ label", (type) => {
    const markers = [
  ...
  it("Int✓Int✗ rechallenge_limit — click → 6 rechallenges → gave up", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.FAILED,
        { reason: "rechallenge_limit", duration_ms: 30000, phase_label: "✓" },
        30000,
      ),
    ];
    expect(buildSummaryFromMarkers(markers)!.label).toBe("Int✓Int✓");
  ...
  it("Int→ Emb→ — both auto-solved (multi-phase)", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.SOLVED,
        { method: "auto_solve", signal: "bridge_solved", duration_ms: 6000, phase_label: "→" },
        6000,
      ),
    ];
    const result = buildSummaryFromMarkers(markers)!;
  ...
  it("Int✓ Emb→ — interstitial clicked, embedded auto", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.SOLVED,
        { method: "auto_solve", signal: "bridge_solved", duration_ms: 8000, phase_label: "→" },
        8000,
      ),
    ];
  ...
  it("Int→ Emb✓ — interstitial auto, embedded clicked", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.SOLVED,
        { method: "click_solve", signal: "beacon_push", duration_ms: 7000, phase_label: "✓" },
        7000,
      ),
    ];
    expect(buildSummaryFromMarkers(markers)!.label).toBe("Int→ Emb✓");
  ...
  it("Int✓Int→ Emb→ — rechallenge + embedded auto", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.SOLVED,
        { method: "auto_solve", signal: "bridge_solved", duration_ms: 11000, phase_label: "→" },
        11000,
      ),
    ];
    const result = buildSummaryFromMarkers(markers)!;
  ...
  it("Int→Int⊘ — rechallenge then verified close", () => {
    const markers = [
      ...
      marker(
        CF_MARKERS.FAILED,
        { reason: "verified_session_close", phase_label: "⊘", cf_verified: true },
        15000,
      ),
    ];
    const result = buildSummaryFromMarkers(markers)!;
...
describe("CloudflareTracker", () => {
  ...
  it("clicked → sets clicked, increments click_count, captures coordinates", () => {
    const tracker = new CloudflareTracker(makeInfo());
    tracker.onProgress("clicked", {
      x: 150,
      y: 250,
      checkbox_to_click_ms: 42,
      phase4_duration_ms: 100,
    });
    const snap = tracker.snapshot();
