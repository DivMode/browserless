...
const simulateBridgePush = (active: ActiveDetection, delay: Duration.Input = "2 seconds") =>
  Effect.sleep(delay).pipe(
    Effect.flatMap(() =>
      active.resolution.solve({
        solved: true,
        type: "turnstile",
        method: "bridge_solved",
        duration_ms: 100,
        attempts: 1,
        auto_resolved: true,
        signal: "bridge_push",
  ...
  const depsLayer = Layer.succeed(
    ...
    SolveDeps.of({
      findAndClickViaCDP: (_active, attempt) => {
        ...
        if (
          config.clickSuccessOnAttempt &&
          captures.clickAttempts >= config.clickSuccessOnAttempt
        ) {
          return Effect.succeed(ClickResult.Verified({ clickDeliveredAt: Date.now() }));
        }
        return Effect.succeed(
          config.clickSuccess
            ? ClickResult.Verified({ clickDeliveredAt: Date.now() })
            : ClickResult.NoCheckbox(),
        );
      },
