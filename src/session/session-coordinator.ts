...
export class SessionCoordinator {
  ...
  setupSessionEffect(
    ...
    return Effect.fn("coordinator.setupSession")(function* () {
      ...
      const sendViaSession = (
        ...
        Effect.runPromise(
          Deferred.await(sessionDeferred).pipe(
            Effect.flatMap((s) =>
              Effect.tryPromise(() => s.sendCommand(method, params ?? {}, cdpSid, timeoutMs)),
            ),
          ),
        );
        ...
        (targetId: TargetId, tag: string, payload?: object) => {
          ...
          if (sessionRef) {
            ...
            Effect.runPromise(
              Deferred.await(sessionDeferred).pipe(
                Effect.flatMap((s) =>
                  Effect.sync(() => s.injectMarkerByTargetId(targetId, tag, payload)),
                ),
                Effect.ignore,
              ),
            );
      ...
      const cloudflareHooks: CloudflareHooks = {
        ...
        onPageNavigated: (tid, sid, url, title) =>
          cloudflareSolver.onPageNavigated(tid, sid, url, title),
        onIframeAttached: (tid, sid, url, parentTid) =>
          cloudflareSolver.onIframeAttached(tid, sid, url, parentTid),
      ...
      yield* Effect.tryPromise({
        ...
        Effect.catch((e: unknown) =>
          Deferred.fail(sessionDeferred, e instanceof Error ? e : new Error(String(e))).pipe(
            ...
            Effect.tap(() =>
              Effect.logWarning(
                `Failed to setup session: ${e instanceof Error ? e.message : String(e)}`,
              ),
            ),
            Effect.tap(() => Effect.sync(() => coordinator.cloudflareSolvers.delete(sessionId))),
            Effect.tap(() =>
              Effect.tryPromise(() => cdpSession.destroy("error")).pipe(Effect.ignore),
            ),
          ),
        ),
      );
  ...
  stopReplayEffect(
    ...
    return Effect.fn("coordinator.stopReplay")(function* () {
      ...
      Effect.ensuring(
        Effect.fn("coordinator.stopReplay.cleanup")(function* () {
   ...
   */
  getReplayMarkerCallback(
    sessionId: string,
  ): ((targetId: TargetId, tag: string, payload?: object) => void) | undefined {
    const session = this.cdpSessions.get(sessionId);
    if (!session) return undefined;
